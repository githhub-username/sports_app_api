...
app.get("/list", async (req, resp) => {
    ...
    const formattedData = data.map(item => ({
        ...item.toObject(),
        images: item.images.map(image => `${req.protocol}://${req.get('host')}/merch_images/${image}`) // Correct URL mapping
    }));

    resp.send(formattedData);
});

// Create new merchandise with multiple image uploads
app.post("/create", upload.array('merch_images', 5), async (req, resp) => {
    try {
        const images = req.files.map(file => file.filename); // Only save filenames, not full paths
        ...
        // Send back the response with the full URL for the images
        resp.send({
            ...result.toObject(),
            images: images.map(image => `${req.protocol}://${req.get('host')}/merch_images/${image}`) // Construct full URL here
        });
    } catch (error) {
        console.error(error);
...
app.put("/update/:_id", upload.array('merch_images', 5), async (req, resp) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            const imagePaths = req.files.map(file => file.filename); // Only save filenames, not full paths
            updatedFields.images = imagePaths; // No full URL here, just filenames
        }
        ...
        // Send the response with full URLs for the images
        const formattedItem = {
            ...updatedItem.toObject(),
            images: updatedItem.images.map(image => `${req.protocol}://${req.get('host')}/merch_images/${image}`) // Construct full URL here
        };
